            ...
            if peerId.namespace == Namespaces.Peer.SecretChat {
                ...
                if isForward {
                    ...
                    if forwardPeerIds.count != 1 {
                        ...
                        Logger.shared.log("GiftForwardProbe", "messages.forwardMessages request grouped=true fromPeerId=\(inputSourcePeerId) toPeerId=\(messages[0].0.id.peerId) ids=\(forwardIds.map { $0.0.id }) hideNames=\(hideSendersNames) hideCaptions=\(hideCaptions) topMsgId=\(String(describing: topMsgId))")
