        ...
        if !hasArrow {
            ...
            if #available(iOS 26.0, *) {
                if let window = UIApplication.shared.connectedScenes
                    ...
                    .flatMap({ $0.windows })
                    .first {
                    self.backgroundMaskNode.layer.rasterizationScale = window.screen.scale
                } else {
                    self.backgroundMaskNode.layer.rasterizationScale = UITraitCollection.current.displayScale
                }
            } else {
