...
private func isGiftTransferProbeEnabled() -> Bool {
#if DEBUG
    return CommandLine.arguments.contains("--gift-transfer-probe")
#else
    return false
#endif
}
        ...
        init(
            ...
            self.disposable = combineLatest(
                ...
                if peerId != context.account.peerId {
                    ...
                    if let disallowedGifts, disallowedGifts.contains(.unique), !isGiftTransferProbeEnabled() {
                    } else {
                        if let disallowedGifts, disallowedGifts.contains(.unique), isGiftTransferProbeEnabled() {
                            Logger.shared.log("StarGiftProbe", "Bypassing unique transfer UI filter for peerId=\(peerId) disallowedGifts=\(disallowedGifts.rawValue)")
                        }
