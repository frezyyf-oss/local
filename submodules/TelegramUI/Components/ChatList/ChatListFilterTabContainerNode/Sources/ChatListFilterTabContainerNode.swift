...
public final class ChatListFilterTabContainerNode: ASDisplayNode {
    ...
    private let themedBackgroundView: EahatGramChatListThemeBackgroundView
    private let themedSelectedBackgroundView: EahatGramChatListThemeBackgroundView
    private let themeLongPressGesture: UILongPressGestureRecognizer
    ...
    public var themeElementLongPressed: ((ExperimentalUISettings.ChatListCustomThemeElement, UIView?) -> Void)?
    ...
    public init(context: AccountContext) {
        ...
        self.themedBackgroundView = EahatGramChatListThemeBackgroundView()
        self.themedSelectedBackgroundView = EahatGramChatListThemeBackgroundView()
        self.themeLongPressGesture = UILongPressGestureRecognizer()
    ...
    private var currentSelectedBackgroundFrame: CGRect?

    @objc private func themeLongPressGestureRecognized(_ gesture: UILongPressGestureRecognizer) {
        guard gesture.state == .began else {
            return
        }
        guard let currentParams = self.currentParams, !currentParams.isReordering, !currentParams.isEditing else {
            return
        }
        guard let themeElementLongPressed = self.themeElementLongPressed else {
            return
        }

        let locationInScrollView = gesture.location(in: self.scrollNode.view)
        if let currentSelectedBackgroundFrame = self.currentSelectedBackgroundFrame, currentSelectedBackgroundFrame.contains(locationInScrollView) {
            themeElementLongPressed(.selectedFolder, self.themedSelectedBackgroundView)
            return
        }

        let locationInBackground = gesture.location(in: self.backgroundView.contentView)
        if CGRect(origin: CGPoint(), size: self.backgroundView.contentView.bounds.size).contains(locationInBackground) {
            themeElementLongPressed(.foldersStrip, self.themedBackgroundView)
        }
    }
    ...
    public func update(size containerSize: CGSize, sideInset containerSideInset: CGFloat, filters: [ChatListFilterTabEntry], selectedFilter: ChatListFilterTabEntryId?, isReordering: Bool, isEditing: Bool, canReorderAllChats: Bool, filtersLimit: Int32?, transitionFraction: CGFloat, presentationData: PresentationData, transition proposedTransition: ContainedViewLayoutTransition) {
        ...
        let foldersStripThemeValue = eahatGramResolvedChatListThemeValue(
            settings: settings,
            element: .foldersStrip,
            isDark: presentationData.theme.overallDarkAppearance
        )
        let selectedFolderThemeValue = eahatGramResolvedChatListThemeValue(
            settings: settings,
        ...
        self.backgroundView.update(
            ...
            isDark: presentationData.theme.overallDarkAppearance,
            tintColor: .init(kind: foldersStripThemeValue == nil ? .panel : .clear),
            isInteractive: true,
            transition: ComponentTransition(transition)
        )
        transition.updateFrame(view: self.themedBackgroundView, frame: CGRect(origin: CGPoint(), size: backgroundSize))
        self.themedBackgroundView.update(
            value: foldersStripThemeValue,
            isDark: presentationData.theme.overallDarkAppearance,
            cornerRadius: backgroundSize.height * 0.5
        )
        self.themedBackgroundView.isHidden = foldersStripThemeValue == nil
        ...
        for i in 0 ..< tabSizes.count {
            ...
            contentWidth += paneNodeSize.width
            if i != tabSizes.count - 1 {
                contentWidth += minSpacing
            }
        }
        let longTitlesWidth = contentWidth + resolvedSideInset * 2.0
        let useShortTitles = longTitlesWidth > backgroundSize.width
        let centeredInset = max(resolvedSideInset, floor((backgroundSize.width - contentWidth) * 0.5))
        var leftOffset: CGFloat = useShortTitles ? resolvedSideInset : centeredInset
        ...
        leftOffset += useShortTitles ? resolvedSideInset : centeredInset
        ...
        if let selectedFrame = selectedFrame {
            ...
            self.currentSelectedBackgroundFrame = selectedBackgroundFrame
            transition.updateFrame(view: self.themedSelectedBackgroundView, frame: selectedBackgroundFrame)
            self.themedSelectedBackgroundView.update(
                value: selectedFolderThemeValue,
                isDark: presentationData.theme.overallDarkAppearance,
                cornerRadius: selectedBackgroundFrame.height * 0.5
            )
            self.themedSelectedBackgroundView.isHidden = selectedFolderThemeValue == nil

            if selectedFolderThemeValue == nil {
            ...
            self.selectedBackgroundNode.isHidden = true
            self.themedSelectedBackgroundView.isHidden = true
            self.previousSelectedAbsFrame = nil
            self.previousSelectedFrame = nil
            self.currentSelectedBackgroundFrame = nil
        }
    }
}
