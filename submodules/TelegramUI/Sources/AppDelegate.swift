...
private let earlyCrashLogPath: String = NSHomeDirectory() + "/Library/Caches/launch-crash.log"

private func writeEarlyCrashLine(_ text: String) {
    let line = "[\(Date())] \(text)\n"
    guard let data = line.data(using: .utf8) else { return }
    let url = URL(fileURLWithPath: earlyCrashLogPath)
    if FileManager.default.fileExists(atPath: earlyCrashLogPath), let handle = try? FileHandle(forWritingTo: url) {
    ...
    NSLog("EARLY_CRASH %@", text)
}

private var earlyCrashHandlersInstalled = false

private func installEarlyCrashHandlers() {
    if earlyCrashHandlersInstalled { return }
    earlyCrashHandlersInstalled = true

    NSSetUncaughtExceptionHandler { exception in
        var lines: [String] = []
        lines.append("EARLY_CRASH NSException name=\(exception.name.rawValue)")
        ...
        writeEarlyCrashLine(lines.joined(separator: "\n"))
    }

    let signals: [Int32] = [SIGABRT, SIGSEGV, SIGBUS, SIGILL, SIGFPE, SIGTRAP]
    for sig in signals {
        signal(sig) { signo in
            NSLog("EARLY_CRASH signal=%d", signo)
            for symbol in Thread.callStackSymbols {
                NSLog("EARLY_CRASH   %@", symbol as NSString)
            }
...
private func isKeyboardWindow(window: NSObject) -> Bool {
    ...
    if #available(iOS 9.0, *) {
        // iOS 26 might have renamed these classes, so check multiple patterns
        if typeName.hasPrefix("UI") && (typeName.hasSuffix("RemoteKeyboardWindow") || typeName.contains("KeyboardWindow")) {
            return true
        }
    } else {
        if typeName.hasPrefix("UI") && (typeName.hasSuffix("TextEffectsWindow") || typeName.contains("TextEffects")) {
            return true
        }
    }
    return false
...
private func isKeyboardView(view: NSObject) -> Bool {
    ...
    // iOS 26 might have renamed these classes
    if typeName.hasPrefix("UI") && (typeName.hasSuffix("InputSetHostView") || typeName.contains("InputSetHost") || typeName.contains("InputHost")) {
        return true
    }
    return false
}
...
private func isKeyboardViewContainer(view: NSObject) -> Bool {
    let typeName = NSStringFromClass(type(of: view))
    // iOS 26 might have renamed these classes
    if typeName.hasPrefix("UI") && (typeName.hasSuffix("InputSetContainerView") || typeName.contains("InputSetContainer") || typeName.contains("InputContainer")) {
        return true
    }
    return false
}
...
@objc(AppDelegate) class AppDelegate: UIResponder, UIApplicationDelegate, PKPushRegistryDelegate, UNUserNotificationCenterDelegate, URLSessionDelegate, URLSessionTaskDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]? = nil) -> Bool {
        ...
        if testIsLaunched {
            writeEarlyCrashLine("didFinishLaunchingWithOptions invoked twice — skipping second invocation")
            return true
        }
        testIsLaunched = true
        ...
        // iOS 26 strictly requires view hierarchy modifications on main thread
        assert(Thread.isMainThread, "MetalEngine.shared.rootLayer must be added on main thread")
        ...
        let networkArguments = NetworkInitializationArguments(
            ...
            isICloudEnabled: buildConfig.isICloudEnabled && isUsingAppGroupContainer
        )
