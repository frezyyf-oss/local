...
private let eahatGramAutoReplyHandledMessageIds = Atomic<Set<String>>(value: Set())
...
public final class ChatHistoryListNodeImpl: ListViewImpl, ChatHistoryNode, ChatHistoryListNode {
    ...
    private func eahatGramProcessAutoReplyMessages(transition: ChatHistoryListViewTransition) {
        guard let previousHistoryView = self.historyView, transition.options.contains(.AnimateInsertion), let peerId = self.chatLocation.peerId else {
            return
        }
        ...
        var previousMessageIds = Set<MessageId>()
        var previousMaxIndex: MessageIndex?
        for entry in previousHistoryView.filteredEntries {
            switch entry {
            case let .MessageEntry(message, _, _, _, _, _):
                previousMessageIds.insert(message.id)
                if message.id.namespace == Namespaces.Message.Cloud {
                    if let currentPreviousMaxIndex = previousMaxIndex {
                        if currentPreviousMaxIndex < message.index {
                            previousMaxIndex = message.index
                        }
                    } else {
                        previousMaxIndex = message.index
                    }
            ...
            case let .MessageGroupEntry(_, messages, _):
                for message in messages {
                    previousMessageIds.insert(message.0.id)
                    if message.0.id.namespace == Namespaces.Message.Cloud {
                        if let currentPreviousMaxIndex = previousMaxIndex {
                            if currentPreviousMaxIndex < message.0.index {
                                previousMaxIndex = message.0.index
                            }
                        } else {
                            previousMaxIndex = message.0.index
                        }
        ...
        for entry in transition.historyView.filteredEntries {
            switch entry {
            case let .MessageEntry(message, _, _, _, _, _):
                let isAfterPreviousMax = previousMaxIndex.map { $0 < message.index } ?? true
                if !previousMessageIds.contains(message.id), isAfterPreviousMax {
                    insertedMessages.append(message)
                }
            case let .MessageGroupEntry(_, messages, _):
                for message in messages {
                    let isAfterPreviousMax = previousMaxIndex.map { $0 < message.0.index } ?? true
                    if !previousMessageIds.contains(message.0.id), isAfterPreviousMax {
                        insertedMessages.append(message.0)
                    }
                }
            default:
                break
        ...
        for message in insertedMessages {
            ...
            guard message.effectivelyIncoming(self.context.account.peerId) else {
                continue
            }
            ...
            let _ = eahatGramAutoReplyHandledMessageIds.modify { current in
                var updated = current
                if !updated.contains(messageKey) {
                    updated.insert(messageKey)
                    shouldReply = true
                }
                return updated
            }
            ...
            eahatGramRequestAutoReply(incomingText: incomingText) { [weak self] result in
                Queue.mainQueue().async {
                    guard let self else {
                        return
                    }
                    ...
                    let currentSettings = self.context.sharedContext.immediateExperimentalUISettings
                    ...
                    let _ = (enqueueMessages(account: self.context.account, peerId: peerId, messages: [replyMessage])
    ...
    private func dequeueHistoryViewTransitions() {
        ...
        self.eahatGramProcessAutoReplyMessages(transition: transition)
