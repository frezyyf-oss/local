        ...
        let makeBody: () -> CGSize = { [weak local] in
                    ...
                    action: { [weak call, weak state] in
                        ...
                        final class ReferenceContentSource: ContextReferenceContentSource {
                            ...
                            func transitionInfo() -> ContextControllerReferenceViewInfo? {
                                ...
                                return ContextControllerReferenceViewInfo(referenceView: self.sourceView, contentAreaInScreenSpace: bounds)
                            }
                        }
            ...
            if #available(iOS 26.0, *) {
                if let window = UIApplication.shared.connectedScenes
                    ...
                    .flatMap({ $0.windows })
                    .first {
                    imageRenderScale = window.screen.scale
                } else {
                    imageRenderScale = UITraitCollection.current.displayScale
                }
            } else {
                imageRenderScale = UIScreen.main.scale
            }
